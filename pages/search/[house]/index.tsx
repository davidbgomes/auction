...
export default function House({house} : any) : JSX.Element {

  const renderer = ({ days, hours, minutes, seconds, completed } : { days: number, hours: number, minutes: number, seconds: number, completed: boolean}) => {
    if (completed) {
      return <Text>Terminado</Text>;
    } else {
      return <span>{days && `${days} ${days > 1 ? 'dias' : 'dia'} `}{("0" + hours).slice (-2)}:{("0" + minutes).slice (-2)}:{("0" + seconds).slice (-2)}</span>;
    }
  };
  ...
  return(
    <Container maxW="container.xl" py="2">
      <Grid
        templateRows={{base:"1fr 1fr 450px 450px 30px", lg:"40% repeat(2, 1fr)"}}
      ...
      >
        ...
        >
          ...
          <Box d="flex" pos="absolute" bottom="0" bgColor="#7ca9cd2e" w="full" height="70px" alignItems="center" justifyContent="center">
            <Text fontSize="xl"><b>Lance Atual:</b> {currentBid > 0 ? <CurrencyField value={currentBid} /> : '-'}</Text>
          </Box>
        </GridItem>
        <GridItem
          rowSpan={1}
        ...
        >
          ...
          <Divider />
          <MapLeaflet />
          <Text fontSize="xs" py="5" px="3">Rua Guilherme Marconi, Nº 10 - 4º Direito <br></br> 2620-448 <br></br> Odivelas, União de Freguesias Ramada e Caneças </Text>
        </GridItem>
        <GridItem
          rowSpan={1}
          colSpan={{base:4, lg:1}}
