...
export default function House({house} : {house : HouseType} ) : JSX.Element {
  ...
  const renderer = ({ days, hours, minutes, seconds, completed } : { days: number, hours: number, minutes: number, seconds: number, completed: boolean}) => {
    if (completed) {
      ...
      return <span>{days > 0 && (`${days} ${days > 1 ? 'dias' : 'dia'} `)}{("0" + hours).slice(-2)}:{("0" + minutes).slice(-2)}:{("0" + seconds).slice(-2)} h</span>;
    }
  };
  ...
  return(
    <>
      ...
      <Container maxW="container.xl" py="2">
        <Grid
          templateRows={{base:"1fr 1fr 450px 450px 30px", lg:"500px 1fr 50px"}}
              ...
              <VStack spacing={{base:4, md:7}} w="full">
                ...
                <SimpleGrid columns={{base:1, md: 4}} spacing={{base:1, md: 5}} w="full">
                  <Box d="flex" alignItems={{base:"flex-start", md:"center"}} mr="auto" m={{base:"inherit", md:"auto"}} flexDirection={{base:"inherit", md:"column"}} w="full" >
                    ...
                    <Text fontSize="md" ml={{base:"2", md:"0"}}>{`${district}, ${county}`}</Text>
                  </Box>
                  <Box d="flex" alignItems={{base:"flex-start", md:"center"}} mr="auto" m={{base:"inherit", md:"auto"}} flexDirection={{base:"inherit", md:"column"}} w="full" >
                    <Icon as={houseType === 'Apartamento' ? BiBuilding : BiHome} w={{base:6, md:8}} h={{base:6, md:8}}/>
                    <Text fontSize="md" ml={{base:"2", md:"0"}}>{houseType}</Text>
                  </Box>
                  <Box d="flex" alignItems={{base:"flex-start", md:"center"}} mr="auto" m={{base:"inherit", md:"auto"}} flexDirection={{base:"inherit", md:"column"}} w="full" >
                    <Icon as={BiBed} w={{base:6, md:8}} h={{base:6, md:8}}/>
                    <Text fontSize="md" ml={{base:"2", md:"0"}}>{typology}</Text>
                  </Box>
                  <Box d="flex" alignItems={{base:"flex-start", md:"center"}} mr="auto" m={{base:"inherit", md:"auto"}} flexDirection={{base:"inherit", md:"column"}} w="full" >
                    <Icon as={BiArea} w={{base:6, md:8}} h={{base:6, md:8}}/>
                    <Text fontSize="md" ml={{base:"2", md:"0"}}>{area}  m²</Text>
                  </Box>
                </SimpleGrid>
              </VStack>
              <Box px={{md:"6"}}>
                <Text fontSize={{base:"sm", md:"md"}} textAlign="justify">{description}</Text>
              </Box>
              <Box px={{md:"6"}} w="full">
                <Text fontSize={{base:"sm", md:"md"}}> <b>Leiloeira:</b> {website}</Text>
              </Box>
            </VStack>
          </GridItem>
          <GridItem
            ...
            pos="relative"
          >
            <Heading textAlign="center" fontSize={{base:"xl", md:"2xl"}} fontWeight="normal" my="3">Detalhes</Heading>
            <Divider />
            <VStack mt="8" spacing="7" alignItems="flex-start" mx={{base:"4", lg:"1", xl:"7"}}>
            ...
            <Box d="flex" pos="absolute" bottom="0" bgColor="#7ca9cd2e" w="full" height="70px" alignItems="center" justifyContent="center">
              <Text fontSize="xl"><b>Lance Atual:</b> {currentBid && parseInt(currentBid.toString()) > 0 ? <CurrencyField value={currentBid} /> : '-'}</Text>
            </Box>
          </GridItem>
          <GridItem
            rowSpan={1}
            ...
            borderRadius="md"
          >
            <Heading textAlign="center" fontSize={{base:"xl", md:"2xl"}} fontWeight="normal" my="3">Localização</Heading>
            <Divider />
            <MapLeaflet latitude={parseFloat(latitude)} longitude={parseFloat(longitude)} title={title}/>
            <Text fontSize="xs" py="5" px="3">
              {`${addressLine1}${addressNumber !== null ? `, ${addressNumber}` : ''}${addressFloor !== null ? ` ${addressFloor}` : ''} `} <br></br> {postcode} <br></br> {county}, {parish}
              </Text>
          </GridItem>
          <GridItem
            rowSpan={1}
